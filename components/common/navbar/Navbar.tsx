...
const Navbar: FC = () => {
  ...
  // Toggle sticky navbar shadow on scroll
  useEffect(() => {
    const scrollHandler = () => {
  ...
  // On mount or when activeTab changes, scroll to the section if hash is present in URL
  useEffect(() => {
  ...
  }, []); // Run once on mount, not on activeTab change

  // Toggle sidebar open/close via checkbox input
  ...
  return (
    <header
      className={`justify-center py-1 w-full h-16 md:h-20 relative z-50 ${
        stickyNavbar
        ...
        <div className="w-fit md:z-40">
          <Link href="/">
            <h1 className="text-primary text-2xl md:text-5xl font-semibold font-caveat">
              Deval
              <span className="inline md:hidden lg:inline">entine</span>
              <span className="text-text-light-primary">.</span>
            </h1>
          </Link>
        </div>
        ...
        >
          <ul
            className={`w-72 flex justify-start flex-col md:bg-transparent bg-bg-primary items-start h-full pt-5 gap-y-3 z-30 md:z-auto relative transition-all ease-linear duration-300 delay-500 ${
              showSideMenu ? "left-0 overflow-scroll" : "-left-[1200px]"
