...
const ExperienceCard = ({
  ...
  return (
    ...
    >
      ...
      <span className="  ml-2 md:ml-16 flex flex-col w-full min-h-52 h-fit shadow-md rounded-md p-4 relative max-md:before:hidden before:h-full before:w-28 before:content-[''] before:-left-28 before:bg-gray-50 before:absolute before:top-0 experience-card px-3">
          ...
          </span>
        {
          responsibilities.map((responsibility, index) => (
            <dl key={index}>
              <dt className="font-medium ">{index+1} . {responsibility.name}</dt>
              <dd className="ml-3 relative before:absolute before:h-full before:w-[2px] before:bg-red-500 before:content-[''] before:left-0 pl-1 py-2 flex items-center bg-blue-50 bg-opacity-30">{responsibility.description}</dd>
